...
import { createCategory, getIncomeCategory } from "@/service/transaction";
...
export default function Index() {
  ...
  const handleSaveCategory = async (categoryName: string) => {
    // Call the API with the new category name
    try {
      const newCategory = await createCategory(categoryName);
      setCategories((prev) => [...prev, newCategory]); // Update local state with new category
  ...
  {categories
    .filter(category => category.name && category.name.toLowerCase().includes(searchTerm.toLowerCase()))
    .map((category, index) => (
      <tr key={index}>
